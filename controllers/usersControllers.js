...
class UserControllers {
  async userAdd(req, res, next) {
    try {
      const { lastname, firstname, phone, password, password_rep } = req.body;
  ...
  async user2Add(req, res, next) {
    try {
      ...
      if (!tex_pas_id) {
        return next(ApiError.badRequest("tex_pas_id was not entered"));
      }
      if (!prava) {
  ...
  async userLogin(req, res, next) {
    try {
      ...
      //   if (!phone || !password) {
      //     return next(
      //       ApiError.badRequest("Phone number and password are required.")
      //     );
      //   }
